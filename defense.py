...
**Weighted Adjusted Stop Rate (WASR):** - This goes a step further by weighting those ASR predictions based on how often each field position actually occurred in the real data. It gives a more grounded view of a team's defensive effectiveness considering real-world game flow.
    ...
    # Load data when parameters change or when button is clicked
    if st.sidebar.button("Fetch Data") or 'df' not in locals():
    ...
    with tab2:
        ...
        if st.button("Train Logistic Ridge Regression Model", key="train_button"):
            ...
            test_df = train_asr_model(model_df, team_names, SC)
            asr_df = compute_asr(test_df, df, team_names)
            time.sleep(1)
            
            # Step 3: Filter & Display
            progress.progress(80, text="Postprocessing results...")
            asr_df = asr_df[asr_df['defense'].isin(stop_rate_df['defense'])]  # Only teams with >30 stops
            
            progress.progress(100, text="✅ Done! ASR Model Trained Successfully")
            
            # Display results
            st.markdown("### Top 10 Teams by Adjusted Stop Rate (ASR)")
            st.table(asr_df.head(10).sort_values('Weighted ASR%', ascending=False).reset_index(drop=True).style.format({
                "ASR%": "{:.2f}",
                "Weighted ASR%": "{:.2f}"
            }))
    ...
    with tab4:
        ...
        if choice == 'Train (VS) Test Stop Rate':
            full_df = load_and_preprocess_data(year, 15)
            train_df = full_df[full_df['week'] < 9]
            test_df = full_df[full_df['week'] >= 9]
            
            st.markdown(f"### Train Stop Rate vs Test Stop Rate Comparison ({year} Season)")
            
            team_names = list(full_df['defense'].unique())
            ...
            test_df_asr = train_asr_model(train_model_df, team_names, SC)
            asr_df = compute_asr(test_df_asr, train_df, team_names)
